...
// Memory fallback cache for serverless environments (Vercel)
let inMemoryDb: DBData | null = null;

// Determine writable DB path: prefer /tmp on serverless/read-only systems
...
// ----------------------------------------------------
export function generateEbookPdfBuffer(customerName: string, orderId: string): Buffer {
...
// ----------------------------------------------------
export async function sendDeliveryEmail(order: OrderRecord, downloadUrl: string) {
  ...
  try {
    ...
    const appUrl = process.env.APP_URL || process.env.VERCEL_URL 
      ? (process.env.APP_URL || `https://${process.env.VERCEL_URL}`).replace(/\/$/, '')
      : '';
